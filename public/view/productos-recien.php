...
if (isset($_REQUEST['id'])) {
    ...
    $queryPromociones = "SELECT id_oferta, nombre_ofert, precio_normal, precio_des, im_oferta, des_ofert FROM promociones WHERE id_oferta = ?";

    // Preparar y ejecutar consulta para obtener detalles de promoción
    $stmtPromociones = $conexion->prepare($queryPromociones);
    $stmtPromociones->bind_param('i', $productoId);
    $stmtPromociones->execute();
    $resultPromociones = $stmtPromociones->get_result();


    // Verifica si la consulta fue exitosa para promociones
    if ($resultPromociones && $resultPromociones->num_rows > 0) {
        $dataPromociones = $resultPromociones->fetch_assoc();
    } else {
        echo "Error en la consulta de promociones: " . $conexion->error;
    }
} else {
...
$descripcion = $dataPromociones["des_ofert"];
        ...
        <section class="main-container pt-15px">
            ...
            <div class="breadcrumbs-row">
                <section class="breadcrumbs">
                    <a href="../../index.php" style="color: var(--text-color-light);">
        ...
        <section class="main-container product-details-grid bg-white">
                ...
                <section class="product-g" id="product-g-mobile">
                    <div class="gallery-container">
                        <div class="thumbnail-container">
                            <img src="data:image/jpg;base64,<?php echo base64_encode($dataPromociones['im_oferta']); ?>" alt="Imagen miniatura del producto" class="selected-thumbnail">

                        </div>
                        <div class="main-img">
                            <img src="data:image/jpg;base64,<?php echo base64_encode($dataPromociones['im_oferta']); ?>" alt="Imagen del producto">
                        </div>
                    </div>
                </section>

                <section class="product-details">
                    <div class="zoom-lens" id="zoom" style="<?php echo "background-image: url('data:image/jpg;base64," . base64_encode($dataPromociones['im_oferta']) . "');"; ?>"></div>

                    <div class="product-title ">
                        <h1><?php echo $dataPromociones["nombre_ofert"] ?></h1>

                    </div>

                    <div class="product-price">
                        <h2>
                            S/ <?php echo $dataPromociones['precio_des'] ?>
                        </h2>
                    </div>

                    <!--<div class="payment-twelve-months">-->
                    <!--    S/ <?php echo $dataPromociones['precio_normal'] ?>-->
                    <!--</div>-->

                    <ul class="card__list grid">
                        <?php
                        $array_des = explode("\n", $dataPromociones['des_ofert']);
                        if (!empty($dataPromociones['des_ofert'])) {
                        ?>
                            <ul class="card__list grid">
                                <?php
                    ...
                    <script>
                        document.getElementById('comprarAhoraBtn').addEventListener('click', function() {
                            ...
                            var nombreProducto = '<?php echo $dataPromociones["nombre_ofert"]; ?>';

                            // Construye el mensaje con la cantidad seleccionada y el nombre del producto
                            var mensaje = 'Hola, quiero comprar ' + nombreProducto + '. ¿Puedes proporcionarme más detalles?';
            ...
            <article class="left-column">
                <section class="product-g" id="product-g-desktop">
                    <div class="gallery-container">
                        <div class="thumbnail-container">
                            <img src="data:image/jpg;base64,<?php echo base64_encode($dataPromociones['im_oferta']); ?>" onmouseover="changeImage('data:image/jpg;base64,<?php echo base64_encode($dataPromociones['im_oferta']); ?>', 1)" class="selected-thumbnail" id="pic1">
                        </div>
                        <div class="main-img" id="picture" onmousemove="move(event)">
                            <div class="rect" id="rect"></div>
                            <img src="data:image/jpg;base64,<?php echo base64_encode($dataPromociones['im_oferta']); ?>" alt="Imagen del producto" id="pic">
                        </div>
                    </div>
                </section>
                ...
                <div class="mx-30px">
                    <section>
                        ...
                        <table class="features-table">
                            <tbody>
                                <tr>
                                    <th class="feature-title">Nombre del producto</th>
                                    <td class="feature-value"><?php echo $dataPromociones["nombre_ofert"] ?></td>
                                </tr>
                                <?php
