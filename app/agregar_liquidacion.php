...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["name"]) && !empty($_POST["name"]) && isset($_POST["precio"]) && !empty($_POST["precio"])) {
        ...
        if (isset($_FILES['txtimg']) && $_FILES['txtimg']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array(strtolower($fileExtension), $allowedExtensions)) {
                ...
                $query = "INSERT INTO nuevo_liquidaccion (nombre_nuevo, precio_product, img_nuevo) VALUES (?, ?, ?)";
                ...
                if ($stmt) {
                    // Enlazar los parámetros con las variables
                    $stmt->bind_param("sss", $nombreLiqui, $precio, $imgData);

                    if ($stmt->execute()) {
                        // La empresa se ha agregado con éxito
                        $alert = '<div class="alert success">Liquidacion agregada con éxito.</div>';
                    } else {
                        // Hubo un error al agregar la empresa
                        $alert = '<div class="alert error">Error al agregar la liquidacion: ' . $stmt->error . '</div>';
                    }
                    $stmt->close();
                } else {
        ...
        $alert = '<div class="alert error">Por favor, ingresa un precio válido.</div>';
    }
}
?>
