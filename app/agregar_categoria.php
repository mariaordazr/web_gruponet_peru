...
if (isset($_POST["btnAgregar"])) {
    ...
    if (isset($_POST["name"]) && !empty($_POST["name"])) {
        ...
        $insertQuery = "INSERT INTO categoria (name_categoria) VALUES (?)";
        $stmt = $conexion->prepare($insertQuery);
        $stmt->bind_param("s", $nombreMarca);

        if ($stmt->execute()) {
            // La marca se ha agregado con éxito
            $alert = '<div class="alert success">Categoria agregada con éxito.</div>';
        } else {
            // Hubo un error al agregar la marca
            $alert = '<div class="alert error">Error al agregar la Categoria.</div>';
        }
    } else {
        // Si el campo "name" del formulario de agregar está vacío
        $alert = '<div class="alert error">Por favor, ingresa un nombre de categoria válida.</div>';
    }
}
